    ...
    if (message.type === 'utf8') {
      ...
      if (
        ...
        if (compiledWidth > monitorSize.width) {
          ...
          console.log(`Requested width (${compiledWidth}) exceeds monitor bounds (${monitorSize.width}), limiting to ${data.args.width}`);
        }
        ...
        console.log(`Taking screenshot at ${data.args.x}, ${data.args.y}, ${data.args.width}, ${data.args.height}`);

        var image = robot.screen.capture(data.args.x, data.args.y, data.args.width, data.args.height);
      ...
      if (data.action === 'move-mouse' && data.args.hasOwnProperty('x') && data.args.hasOwnProperty('y')) {
