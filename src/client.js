...
(async () => {
  ...
  wsp.sendRequest({ action: 'move-mouse', args: { x: pos.x + 10, y: pos.y + 10 } });
  ...
  await wsp
    .sendRequest({ action: 'take-screenshot', args: { x: pos.x, y: pos.y, width: 300, height: 300 } }) // actually sends {foo: 'bar', id: 'xxx'}, because `attachRequestId` defined above
    .then(response => {
      let image = response;
